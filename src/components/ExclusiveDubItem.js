...
import {getNamavaUrl,fetchBriefData} from '../utils/functions';
...
const getCaption = (info) => {
    ...
    if(info['episodCaption']) {
        ...
        caption = <React.Fragment>
            <div className="series-title">{parts[0]}</div>
...
const ExclusiveDubItem = ({item,placeholder = false, togglePreview}) => {
    ...
    return (
        <div className='exclusive-dub-item' onClick={() => {
            if(placeholder !== false) {
                return false;
            }
